...
As expressões programa de afiliados, afiliação, indicação e parceria podem ser utilizadas como termos descritivos e de intenção de busca. Elas não representam um segundo programa institucional.
...
Quando aplicável, esses mecanismos podem apresentar informações relacionadas a indicações, clientes indicados, status, validação, comissões e operação da parceria.
...
A continuidade da comissão depende da permanência do cliente, das regras vigentes, dos critérios de validação e das condições aplicáveis ao Programa.
...
A página principal apresenta o Programa, explica funcionamento, apresenta jornada, esclarece condições essenciais e direciona para o FAQ.

O FAQ responde dúvidas específicas, amplia cobertura de intenção, mantém granularidade Question/Answer e referencia a mesma entidade Programa.

Schema da página do Programa

O Schema da página principal foi atualizado para refletir Organization, WebSite, WebPage, Service, HowTo, ServiceChannel, RegisterAction, Offer do Plano Padrão, DefinedTermSet, DefinedTerm, referências externas públicas, Wikidata e FAQ relacionado.
...
Cada pergunta utiliza identificadores estáveis #faq-01 a #faq-35, com respostas correspondentes #answer-01 a #answer-35.
...
A documentação passa a considerar explicitamente recuperabilidade por mecanismos de busca, sistemas de resposta, crawlers, mecanismos de recuperação por IA, ChatGPT Search, Google, Bing/Copilot e outros sistemas compatíveis.

Isso não autoriza criação de conteúdo artificial, keyword stuffing ou relações semânticas sem suporte factual.

OAI-SearchBot / ChatGPT Search

A estratégia de monitoramento passa a incluir OAI-SearchBot, regras oficiais de rastreamento, robots.txt, acessibilidade pública, indexabilidade, bloqueios por CDN/WAF/firewall, alterações de user-agent e mudanças oficiais da OpenAI aplicáveis à recuperação e citação.
...
O faq-parcerias.md deve ser tratado na fase específica dedicada à página de FAQ.
...
Programa de Parcerias DoctorAmo
≠ segundo programa institucional denominado "Programa de Afiliados DoctorAmo"
...
Toda nova afirmação estrutural deve ser distinguida como:

Fonte institucional: informação publicada ou formalmente comunicada pela DoctorAmo.

Fonte técnica: Schema, documentação pública ou infraestrutura oficial.

Fonte externa: fonte independente, pública e identificável.

Inferência: não deve ser registrada como fato sem evidência suficiente.

Regra de atualização futura

Sempre que houver mudança relevante em nome institucional, preço, comissão, requisitos, credenciamento, avaliação, prazo, licença, fluxo, rastreamento, materiais, bônus, regras, FAQ, URLs, Schema, Wikidata, GitHub, crawlers ou integração semântica, registrar a alteração neste arquivo.

Versão histórica anterior

[2.1.0] — 2026-08-19

A versão 2.1.0 introduziu a primeira camada estruturada de credenciamento, condições de adesão, licença de acesso, avaliação de perfil, prazo operacional, documentos especializados, vocabulário canônico e governança via changelog.

Parte das regras daquela versão foi substituída pela versão 3.0.0.

Não devem ser tratadas como regras atuais:

Pessoa Física → 18 anos ou mais
